...
export async function GET(context: APIContext) {
  ...
  const handWrittenLinks = new Set(handWrittenItems.map(i => i.link));

  const generatedItems = articleIndex
    .filter(a => !handWrittenLinks.has(a.href))
    .map(a => ({
      title: a.title,
  ...
  const allItems = [...handWrittenItems, ...generatedItems]
